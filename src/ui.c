...
int search_process(process_info *plist, size_t count, const char *search_term) {
  if (!plist || !search_term || strlen(search_term) == 0)
    return -1;
...
void ui_loop(process_info **process_list_ptr, pthread_mutex_t *mutex) {
  ...
  char key[512] =
      ""; // message texte récapitulant la dernière touche F détectée
  ...
  /* Fenêtre secondaire pour la liste qui permet de manipuler le contenu
   * indépendamment */
  WINDOW *listwin =
      newwin(h - 11, w, 11, 0); // hauteur = h-11, largeur = w, position y = 5
  scrollok(listwin,
           FALSE); // on gère manuellement le scrolling des éléments via la
                   // variable offset donc on désactive le scrolling automatique
  ...
  while (loop) {
    ...
    if (selection >= (int)count) {
      selection = (count > 0 ? (int)count - 1
                             : 0); // si l'index sélection dépasse, on le réduit
    }
    if (offset > selection) {
    ...
    if (listwin) {
      int new_h =
          h - space_value; // conserver la même logique de placement qu'avant
      if (new_h < 1) {
    ...
    printw("Appuyez sur q pour quitter et F1 pour l'aide | Tab: %d | "
           "Selection: %d\n",
           tab, selection);
    ...
    */
    ...
    wclear(listwin);
    int max_lines =
        h - space_value; // nombre de lignes disponibles pour la liste

    /* Boucle d'affichage : on parcourt les processus visibles en tenant compte
     * de l'offset */
    for (int i = offset; i < (int)count && i < offset + max_lines; i++) {
      ...
      if (plist) {
        ...
        switch (tab) {
        case 0: {
          if (plist == NULL || plist[i].pid == 0 || plist[i].name[0] == '\0')
            break;
          ...
          snprintf(truncated_name, sizeof(truncated_name), "%-20.20s",
                   plist[i].name);

          if (plist[i].mem_usage < 1024) {
            snprintf(ram_usage_str, sizeof(ram_usage_str), "%lu B",
                     plist[i].mem_usage);
          } else if (plist[i].mem_usage < 1024 * 1024) {
            snprintf(ram_usage_str, sizeof(ram_usage_str), "%.2f KB",
                     plist[i].mem_usage / 1024.0);
          } else if (plist[i].mem_usage < 1024 * 1024 * 1024) {
            snprintf(ram_usage_str, sizeof(ram_usage_str), "%.2f MB",
                     plist[i].mem_usage / (1024.0 * 1024.0));
          } else {
            snprintf(ram_usage_str, sizeof(ram_usage_str), "%.2f GB",
                     plist[i].mem_usage / (1024.0 * 1024.0 * 1024.0));
          }
          ...
          if (uptime < 60) {
            ...
            snprintf(uptime_str, sizeof(uptime_str), "%ld:%02ld", uptime / 60,
                     uptime % 60);
          } else {
            long hours = uptime / 3600;
            long minutes = (uptime % 3600) / 60;
            long seconds = uptime % 60;
            snprintf(uptime_str, sizeof(uptime_str), "%ld:%02ld:%02ld", hours,
                     minutes, seconds);
          }
          ...
          snprintf(net_info_str, sizeof(net_info_str), "Up:%s/s Down:%s/s",
                   send_str, recv_str);

          wprintw(listwin, "%6d \t%-20s \t%c \t%6.2f%% \t%8s \t%7s \t%s\n",
                  plist[i].pid, truncated_name, state_char, plist[i].cpu_usage,
                  ram_usage_str, uptime_str, net_info_str);
          break;
        }
        case 1:
        ...
        case 3: {
          ...
          format_bytes((float)plist[i].mem_usage, mem_size_str,
                       sizeof(mem_size_str));
          wprintw(listwin, "%6d  MEM: %.2f%% (%s)\n", plist[i].pid, mem_percent,
                  mem_size_str);
          break;
        }
        case 4: {
    ...
    case KEY_F(4): {
      ...
      if (input_dialog("Rechercher un processus (nom ou PID):", search_term,
                       sizeof(search_term)) == 0) {
        pthread_mutex_lock(mutex);
    ...
    case KEY_F(5): {
      ...
      if (plist_pause && selection >= 0) {
        ...
        snprintf(target_name, sizeof(target_name), "%s",
                 plist_pause[selection].name);
        pthread_mutex_unlock(mutex);

        char msg[512];
        if (remote_idx >= 0) {
          snprintf(msg, sizeof(msg),
                   "Mettre en pause le processus distant %s (PID: %d) sur %s ?",
                   target_name, target_pid, g_remote_configs[remote_idx].name);
        } else {
          snprintf(msg, sizeof(msg),
                   "Mettre en pause le processus %s (PID: %d) ?", target_name,
                   target_pid);
        }
        ...
        if (confirm_dialog(msg)) {
          ...
          if (result == 0) {
            ...
            snprintf(msg, sizeof(msg),
                     "Erreur: impossible de mettre en pause le processus %d",
                     target_pid);
            info_dialog(msg, 1);
          }
        }
    ...
    case KEY_F(6): {
      ...
      if (plist_term && selection >= 0) {
        ...
        char target_name[256];
        snprintf(target_name, sizeof(target_name), "%s",
                 plist_term[selection].name);
        pthread_mutex_unlock(mutex);

        char msg[512];
        if (remote_idx >= 0) {
          snprintf(msg, sizeof(msg),
                   "Arreter le processus distant %s (PID: %d) sur %s ?",
                   target_name, target_pid, g_remote_configs[remote_idx].name);
        } else {
        ...
        if (confirm_dialog(msg)) {
          ...
          if (result == 0) {
            snprintf(msg, sizeof(msg), "Signal SIGTERM envoye au processus %d",
                     target_pid);
            info_dialog(msg, 0);
          } else {
            snprintf(msg, sizeof(msg),
                     "Erreur: impossible d'arreter le processus %d",
                     target_pid);
            info_dialog(msg, 1);
          }
        }
    ...
    case KEY_F(7): {
      ...
      if (plist_kill && selection >= 0) {
        ...
        char target_name[256];
        snprintf(target_name, sizeof(target_name), "%s",
                 plist_kill[selection].name);
        pthread_mutex_unlock(mutex);

        char msg[512];
        if (remote_idx >= 0) {
          snprintf(msg, sizeof(msg),
                   "TUER (SIGKILL) le processus distant %s (PID: %d) sur %s ?",
                   target_name, target_pid, g_remote_configs[remote_idx].name);
        } else {
          snprintf(msg, sizeof(msg),
                   "TUER (SIGKILL) le processus %s (PID: %d) ?", target_name,
                   target_pid);
        }
        ...
        if (confirm_dialog(msg)) {
          ...
          if (result == 0) {
            snprintf(msg, sizeof(msg), "Processus %d tue (SIGKILL)",
                     target_pid);
            info_dialog(msg, 0);
          } else {
            snprintf(msg, sizeof(msg),
                     "Erreur: impossible de tuer le processus %d", target_pid);
            info_dialog(msg, 1);
          }
        }
      } else {
    ...
    case KEY_F(8): {
      ...
      if (plist_cont && selection >= 0) {
        ...
        char target_name[256];
        snprintf(target_name, sizeof(target_name), "%s",
                 plist_cont[selection].name);
        pthread_mutex_unlock(mutex);

        char msg[512];
        if (remote_idx >= 0) {
          snprintf(msg, sizeof(msg),
                   "Redemarrer le processus distant %s (PID: %d) sur %s ?",
                   target_name, target_pid, g_remote_configs[remote_idx].name);
        } else {
        ...
        if (confirm_dialog(msg)) {
          ...
          if (result == 0) {
            snprintf(msg, sizeof(msg), "Processus %d redemarre (SIGCONT)",
                     target_pid);
            info_dialog(msg, 0);
          } else {
            snprintf(msg, sizeof(msg),
                     "Erreur: impossible de redemarrer le processus %d",
                     target_pid);
            info_dialog(msg, 1);
          }
        }
    ...
    case KEY_UP:
      if (selection > 0) {
        selection--;
        if (selection < offset)
          offset--; // décale vers le haut si on sort de la fenêtre
      }
      break;
    ...
    case KEY_DOWN:
      if (selection < (int)count - 1) {
        selection++;
        if (selection >= offset + max_lines)
          offset++; // décale vers le bas si on dépasse
      }
      break;
