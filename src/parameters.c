...
parameters_table *manage_arguments(int argc, char *argv[], int *out_count) {
  ...
  parameters_table *given_parameters =
      calloc(PARAMETER_BUFFER_SIZE, sizeof(parameters_table));
  if (!given_parameters)
    return NULL;
  ...
  while ((opt = getopt_long(argc, argv, "hdc:P:l:s:u:p:at:", my_opts, NULL)) !=
         -1) {
    ...
    case 'c':
      ...
      strncpy(given_parameters[parameters_count].parameter_value.str_param,
              optarg, STR_MAX - 1);
      given_parameters[parameters_count]
          .parameter_value.str_param[STR_MAX - 1] = '\0';
      parameters_count++;
      break;

    case 'P':
      given_parameters[parameters_count].parameter_type = PARAM_PORT;
      given_parameters[parameters_count].parameter_value.int_param =
          atoi(optarg);
      parameters_count++;
      break;
...
bool params_validate(parameters_table *params, int params_count) {
  ...
  if (has_all_flag) {
    ...
    if (!has_config_or_server) {
      fprintf(stderr, "ERROR: Option -a/--all requires -c/--remote-config, "
                      "-s/--remote-server, or -l/--login.\n");
      return false;
    }
  }
  ...
  for (int i = 0; i < params_count; i++) {
    ...
    case PARAM_REMOTE_CONFIG:
      if (strlen(param->parameter_value.str_param) == 0) {
        ...
        if (!is_config_file_valid(param->parameter_value.str_param)) {
          fprintf(stderr, "ERROR: Follow this format for the configuration "
                          "file:\nserver_name1:server_adress:port:username:"
                          "password:connexiontype\n");
          return false;
        }
      }
    ...
    case PARAM_PORT:
      if (!has_config_file) {
        if ((param->parameter_value.int_param < 1) ||
            (param->parameter_value.int_param > 65535)) {
          fprintf(stderr,
                  "ERROR: Please enter a port value between 1 and 65535.\n");
          return false;
        }
      }
      break;
    ...
    case PARAM_REMOTE_SERVER:
      if (!has_config_file) {
        ...
        if (has_login) {
          fprintf(
              stderr,
              "ERROR: Cannot use both -l/--login and -s/--remote-server.\n");
          return false;
        }
      }
      break;
    ...
    case PARAM_USERNAME:
      if (!has_config_file) {
        ...
        if (has_login) {
          fprintf(stderr,
                  "ERROR: Cannot use both -l/--login and -u/--username.\n");
          return false;
        }
      }
      break;
    ...
    case PARAM_CONNECTION_TYPE:
      ...
      if (strcmp(type_lower, "ssh") != 0) {
        fprintf(stderr,
                "ERROR: Connection type '%s' is not supported. Only 'ssh' is "
...
bool is_param_type(parameters_table *params, int params_count,
                   parameters_id_table type) {
  bool found = false;
...
int get_int_parameters(parameters_table *params, int params_count,
                       parameters_id_table type) {
  for (int i = 0; i < params_count; i++) {
    if (params[i].parameter_type == type) {
...
int get_string_parameters(parameters_table *params, int params_count,
                          parameters_id_table type, char *string) {
  for (int i = 0; i < params_count; i++) {
    if (params[i].parameter_type == type) {
...
void manual() {
  printf(COLOR_BOLD "NOM\n" COLOR_OFF
                    "\thtop-clone - interactive process viewer\n\n");
  printf(COLOR_BOLD "SYNOPSIS\n" COLOR_OFF "\t[OPTIONS] arguments...\n\n");
  printf(COLOR_BOLD
         "DESCRIPTION\n" COLOR_OFF
         "\tShow all the process in an interactive table in real time.\n\n");
  printf(COLOR_BOLD "OPTION\n" COLOR_OFF);
  printf("\t-h --help\t\tPrint the manual in the terminal.\n");
  printf("\t-d --dry-run\t\tTest access to the list of processes on the local "
         "and/or remote machine without displaying them.\n");
  printf("\t-c --remote-config\tSpecifies the path to the configuration file "
         "containing the SSH connection information on remote machines.\n");
  printf("\t\t\t\tFormat: name:address:port:username:password:connection_type "
         "(ssh or telnet)\n");
  printf("\t-P --port\t\tSpecifies the SSH port to use. If this option is not "
         "specified, the default port 22 is used.\n");
  printf("\t-l --login\t\tSpecifies the connection identifier and the remote "
         "machine. Ex : --login user@remote_server. remote_server is either "
         "the IP or the DNS name of the remote machine.\n");
  printf("\t-s --remote-server\tSpecifies the DNS name or IP of the remote "
         "machine.\n");
  printf("\t-u --username\t\tSpecifies the username to use for the SSH "
         "connection.\n");
  printf("\t-p --password\t\tSpecifies the password to use for the SSH "
         "connection.\n");
  printf(
      "\t-t --connexion-type\tSpecifies the connection type to use for remote "
      "machines (ssh, telnet). Only 'ssh' is currently implemented.\n");
  printf("\t-a --all\t\tSpecifies to the program to collect both the list of "
         "processes on the local machine and remote machines. Used only if the "
         "-c, -s, or -l option is used.\n");
}
